...
public class WarehouseItemsViewHolder extends RecyclerView.ViewHolder {


    public TextView productName, valRate, itemQuantity, itemDescription, warehouse;
    public View parent;
    WarehouseItemsAdapter warehouseItemsAdapter;
    ImageView cross;
    int position;

    public WarehouseItemsViewHolder(@NonNull View itemView) {
        super(itemView);
        parent = itemView;
        productName = itemView.findViewById(R.id.name);
        valRate = itemView.findViewById(R.id.val_rate);
    ...
    public void setData(Context context, WarehouseItem item, ReconciliationItemCallback callback, int position, WarehouseItemsAdapter warehouseItemsAdapter) {
        this.warehouseItemsAdapter = warehouseItemsAdapter;
        ...
        if (item != null && item.getItemCode() != null) {
            ...
            warehouse.setText(item.getWarehouse());
            valRate.setText("$ " + Utils.round(item.getValuationRate().floatValue(), 3));
//
            ...
            cross.setOnClickListener(v -> {
                callback.onDeleteClick(item, position);
            });
        }
    }
        ...
        setQuantity.setOnClickListener(v -> {
            if (quantity.getText().toString() != null && !quantity.getText().toString().isEmpty() && !quantity.getText().toString().equalsIgnoreCase("0")) {
                ...
                if (title.equalsIgnoreCase("rate")) {
                    ...
                    warehouseItemsAdapter.notifyItemChanged(position);

...
public class AddItemRepo implements OnNetworkResponse {


    private static AddItemRepo instance;
    ...
    public static AddItemRepo getInstance() {
        if (instance == null) {
            instance = new AddItemRepo();
        }
        return instance;
    }
    ...
    public void getSearchLink(int requestCode, String searchDoctype, String query, String filters, String text) {
        NetworkCall.make()
                ...
                .enque(Network.apis().getSearchLinkWithFilters(new SearchLinkWithFiltersRequestBody(text, searchDoctype, "0", filters, "Item", query)))
                .execute();
    }
    ...
    public void saveDoc(JSONObject jsonObject) {
        NetworkCall.make()
                ...
                .enque(Network.apis().saveItem(FileUpload.saveDoc(jsonObject, null)))
                .execute();
    }

    @Override
    public void onSuccess(Call call, Response response, Object tag) throws ParseException {
        if ((int) tag == RequestCodes.API.ITEM_GROUP) {
        ...
        } else if ((int) tag == RequestCodes.API.DEFAULT_UNIT_OF_MEASURE) {
            SearchLinkResponse res = (SearchLinkResponse) response.body();
            if (res != null && res.getResults() != null) {
                res.setRequestCode(0);
                res.setRequestCode((int) tag);

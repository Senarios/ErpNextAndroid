...
public class CustomerGroupsRepo implements OnNetworkResponse {

    private static CustomerGroupsRepo instance;
    public MutableLiveData<List<List<String>>> items = new MutableLiveData<>();
    int limitSet;

    public static CustomerGroupsRepo getInstance() {
        if (instance == null) {
            instance = new CustomerGroupsRepo();
        }
        return instance;
    }
    ...
    public void getItemsApi(String docType, int pageLength, boolean isCommentCount, String orderBy, int limitStart) {
        limitSet = limitStart;
        String fields = "[\"`tabCustomer Group`.`name`\",\"`tabCustomer Group`.`owner`\",\"`tabCustomer Group`.`creation`\",\"`tabCustomer Group`.`modified`\",\"`tabCustomer Group`.`modified_by`\",\"`tabCustomer Group`.`_user_tags`\",\"`tabCustomer Group`.`_comments`\",\"`tabCustomer Group`.`_assign`\",\"`tabCustomer Group`.`_liked_by`\",\"`tabCustomer Group`.`docstatus`\",\"`tabCustomer Group`.`parent`\",\"`tabCustomer Group`.`parenttype`\",\"`tabCustomer Group`.`parentfield`\",\"`tabCustomer Group`.`idx`\",\"`tabCustomer Group`.`customer_group_name`\",\"`tabCustomer Group`.`parent_customer_group`\",\"`tabCustomer Group`.`is_group`\"]";
        NetworkCall.make()
                .setCallback(this)
                .setTag(RequestCodes.API.REPORT_VIEW)
                .autoLoadingCancel(Utils.getLoading(MainApp.INSTANCE.getCurrentActivity(), "Loading..."))
                .enque(Network.apis().getReportView(docType, fields, "[[\"Customer Group\",\"owner\",\"=\",\"" + AppSession.get("email") + "\"]]", pageLength, isCommentCount, orderBy, limitSet))
                .execute();
    }

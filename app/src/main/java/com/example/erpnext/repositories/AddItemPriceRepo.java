...
public class AddItemPriceRepo implements OnNetworkResponse {

    private static AddItemPriceRepo instance;
    ...
    public static AddItemPriceRepo getInstance() {
        if (instance == null) {
            instance = new AddItemPriceRepo();
        }
        return instance;
    }
    ...
    public void getSearchLink(int requestCode, String searchDoctype, String query, String filters, String text) {
        NetworkCall.make()
                ...
                .enque(Network.apis().getSearchLinkWithFilters(new SearchLinkWithFiltersRequestBody(text, searchDoctype, "0", filters, "Item Price", query)))
                .execute();
    }
    ...
    public void onSuccess(Call call, Response response, Object tag) throws ParseException {
        if ((int) tag == RequestCodes.API.PRICE_LIST_LINK_SEARCH) {
        ...
        } else if ((int) tag == RequestCodes.API.ITEM_CODE_SEARCH) {
            SearchLinkResponse res = (SearchLinkResponse) response.body();
            if (res != null && res.getResults() != null) {
                res.setRequestCode(0);
                res.setRequestCode((int) tag);
